...
export function activate(context: vscode.ExtensionContext) {
	...
	vscode.workspace.onDidSaveTextDocument(debounce((args: vscode.TextDocumentChangeEvent) => {
		
		if (!isCsvFile(args.document)) return

		if (args.contentChanges.length === 0) return
 ...
 * @param filePath 
 */
function getResourcePath(context: vscode.ExtensionContext, filePath: string): string {
	return `vscode-resource:${path.join(context.extensionPath, filePath)}`
}

function createHtml(context: vscode.ExtensionContext, initialContent: string): string {
...
function isCsvFile(document: vscode.TextDocument) {
